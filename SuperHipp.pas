...
uses
  ...
  System.Classes, Vcl.Graphics, system.Diagnostics, system.TimeSpan,
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.StdCtrls, Vcl.Imaging.GIFImg,
  VirtualTrees, VirtualStringTreeHipp, System.Generics.Collections,
  Vcl.ExtCtrls, ThreadLoadDB, DM, system.Math, VTREditors,system.Types,
  ...
  GR32_Image, RoleButton, Vcl.OleCtrls, PDFium.Frame,
  Vcl.ComCtrls, Vcl.Buttons,

  SynEdit,
  Vcl.AppEvnts, system.DateUtils,
  WMPLib_TLB, NzisThreadFull, System.IOUtils,

  Aspects.Types, SuperObject, superxmlparser,

  Table.Role, Vcl.WinXCtrls, Vcl.Imaging.pngimage,
  Nzis.Nomen.baseCL000, Vcl.ImgList, VCLTee.Control, VCLTee.Grid,
  ...
  Table.Diagnosis, Table.MDN, VirtualStringTreeAspect, Table.EventsManyTimes,
  ...
  SynEditHighlighter, SynHighlighterXML, Vcl.ToolWin,
  System.ImageList,
  FinderFormFMX,
  FMX.Types, System.Bindings.Expression,
  System.Bindings.ExpressionDefaults,

  Vcl.Menus, DbHelper, ADB_DataUnit,
...
//  end;
  ...
  TfrmSuperHip = class(TForm)
    ...
    mniSpisaci: TMenuItem;
    mniDataDoctors: TMenuItem;
    ...
    procedure mniHipClick(Sender: TObject);
...
begin
  ...
  if not FindNode(dfnone) then // трябва да изтрие последно въведените
  begin
...
begin
  ...
  iExamAnal := 0;
  iMdn := 0;
  while iExamAnal < CollExamAnal.Count do
  begin
...
//              end;
          ...
          vChildPat := vChildPat.NextSibling;
          dataChild := pointer(PByte(vChildPat) + lenNode);
...
function TfrmSuperHip.FindNode(DirectionFind: TDirectionFinder): boolean;
var
  node: PVirtualNode;
  data: PAspRec;

  function FindNodeRepeat(DirectionFind: TDirectionFinder): boolean;
  var
  ...
  begin
    ...
    begin
      ...
      begin
        if vtrPregledPat.Text[node, 0].StartsWith(vtrPregledPat.Header.Columns[0].Text + FinderRec.strSearch) then
        begin
          vtrPregledPat.Selected[node] := True;
...
begin
  ...
  Result := FindNodeRepeat(DirectionFind);
...
begin
  ...
  begin
    ...
    Form := FmxWScreen;
    //FmxFormDyn := TfrmXPregled.Create(nil);
    //Form := FmxFormDyn;
   //FmxFormDyn.scldlyt1.OnResize := ScaleDyn;
   // FmxFormDyn.OnMouseWheelVert := FormFMXMouseWheel;
//////////////////////////////////////////
   // FmxFormDyn := TfrmFmxFormDyn.Create(nil);
//    Form := FmxFormDyn;
//    FmxFormDyn := TfrmFmxFormDyn(Form)
  end;
end;
...
begin
   //FmxFormDyn := TfrmXPregled.Create(nil);
//   FmxFormDyn.HandleHip := Self.Handle;
//   FmxFormDyn.OnAfterShow := OnActivateFMX;
//
//   FmxTest := TfrmFMXCalcPregled.Create(nil);
//   FmxTest.CollPreg := CollPregled;
...
procedure TfrmSuperHip.mniHipClick(Sender: TObject);
begin
...
//              end;
  ...
  FmxProfForm.FillRightLYT(datapat);
...
begin
  ...
  ACollPatFDB.Clear;
  FillListPL_NZOKFromDB(uin, AcollpatFromDoctor); // попълва се за  определен УИН
...
begin
  ...
  New(diag.PRecord);
  diag.PRecord.setProp := [Diagnosis_code_CL011, Diagnosis_rank];
...
var
  ...
  gr: TGraphPeriod132;
  Rule88, ACL144_key, Cl132Key, note, Field_cl133, cl028Key, test: string;
...
begin
  ...
  begin
    ...
    begin
      ...
      AspectsLinkPatPregFile.AddNewNode(vvNZIS_PLANNED_TYPE, NZIS_PLANNED_TYPE.DataPos, vPreg, amAddChildLast, TreeLink, linkpos);
      TreeLink.CheckType := ctCheckBox;
      TreeLink.CheckState := csCheckedNormal;
      planedStatus := [TPlanedStatus.psNew];
      TreeLink.Dummy := Byte(planedStatus);// току що създаден план. Не е проверяван и не е показван
      vCl132 := TreeLink;
      ...
      begin
        ...
        begin
          cl22Pos := CL022Coll.GetDataPosFromKey(PR001Coll.getAnsiStringMap(pr001.DataPos, Word(PR001_Activity_ID)));
...
begin
  ...
  begin
    //if Msg.hwnd = FormToHWND(FmxProfForm) then
//    begin
...
begin
  ...
  case Column of
    ...
    begin
      case data.vid of
        vvPatient: vtrPregledPat.Header.Columns[Column].Text := 'ЕГН ';
...
begin
  ...
  if AnsiUpperCase(Text).StartsWith(AnsiUpperCase(Sender.Header.Columns[Column].Text + FinderRec.strSearch)) then
  begin
    p := AnsiUpperCase(Text).IndexOf((AnsiUpperCase(FilterText)));
    ...
    begin
      case data.vid of
        ...
        begin
          ...
          ofset := p^ + CollPatient.posData;
          CellText := IntToStr(ofset) + ' от ' + DateToStr(CollPregled.getDateMap(Data.DataPos, word(PregledNew_START_DATE)));
        end;
        vvNZIS_PLANNED_TYPE:
        begin
...
begin
  vtrPregledPat.Repaint;
  if fmxCntrDyn.FireMonkeyForm = FmxTokensForm then
   FmxTokensForm.FillTokens;
end;
...
begin
  ShowMessage(IntToStr(Screen.ActiveControl.HelpContext));
  //if Msg.HelpInfo.iContextType = 2 then
//  begin
//    SendMessage(Handle, WM_SYSCOMMAND, SC_CONTEXTHELP, 0);
//  end;
end;

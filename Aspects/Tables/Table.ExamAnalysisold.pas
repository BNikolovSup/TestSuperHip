...
begin
  ...
  SaveStreamCommand(TLogicalData16(PRecord.setProp), CollType, toInsert, FVersion, metaPosition + 4);
  case FVersion of
    0:
    begin
...
procedure TExamAnalysisColl.OnSetTextSearchCnk(Chk: TCheckBoxDyn);
begin
 // if Chk.sta then

end;

procedure TExamAnalysisColl.OnSetTextSearchDTEDT(DtEdt: TDateEditDyn);
begin
  if dtEdt.Date = 0 then
  begin
    Exclude(ListForFDB[0].PRecord.setProp, TExamAnalysisItem.TPropertyIndex(dtEdt.Field));
  end
  else
  begin
    include(ListForFDB[0].PRecord.setProp, TExamAnalysisItem.TPropertyIndex(dtEdt.Field));
  end;
  Self.PRecordSearch.setProp := ListForFDB[0].PRecord.setProp;
  //case TExamAnalysisItem.TPropertyIndex(dtEdt.Field) of
//    ExamAnalysis_BIRTH_DATE: ListForFDB[0].PRecord.BIRTH_DATE  := dtEdt.Date;
//  end;
end;

procedure TExamAnalysisColl.OnSetTextSearchEDT(edt: fmx.EditDyn.TEditDyn);
begin
  if edt.Text = '' then
  begin
    Exclude(ListForFDB[0].PRecord.setProp, TExamAnalysisItem.TPropertyIndex(edt.Field));
  end
  else
  begin
    include(ListForFDB[0].PRecord.setProp, TExamAnalysisItem.TPropertyIndex(edt.Field));
...
//  begin
//    case TExamAnalysisItem.TPropertyIndex(edt.Field) of
//      ExamAnalysis_EGN: ListForFDB[0].PRecord.EGN  := edt.Text;
//      ExamAnalysis_FNAME: ListForFDB[0].PRecord.FNAME  := edt.Text;
//      ExamAnalysis_SNAME: ListForFDB[0].PRecord.SNAME  := edt.Text;
//      ExamAnalysis_ID: ListForFDB[0].PRecord.ID  := StrToInt(edt.Text);
//      //ExamAnalysis_BIRTH_DATE: ListForFDB[0].PRecord.BIRTH_DATE  := StrToInt(edt.Text);
//    end;
//  end
//  else
//  begin
//    case TExamAnalysisItem.TPropertyIndex(edt.Field) of
//      ExamAnalysis_EGN: ListForFDB[0].PRecord.EGN  := AnsiUpperCase(edt.Text);
//      ExamAnalysis_FNAME: ListForFDB[0].PRecord.FNAME  := AnsiUpperCase(edt.Text);
//      ExamAnalysis_SNAME: ListForFDB[0].PRecord.SNAME  := AnsiUpperCase(edt.Text);
//      ExamAnalysis_ID: ListForFDB[0].PRecord.ID  := StrToInt(edt.Text);
//      //ExamAnalysis_BIRTH_DATE: ListForFDB[0].PRecord.BIRTH_DATE  := StrToInt(edt.Text);
//    end;
//  end;

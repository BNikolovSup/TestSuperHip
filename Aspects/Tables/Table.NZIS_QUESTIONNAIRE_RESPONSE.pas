...
, NZIS_QUESTIONNAIRE_RESPONSE_PR001_KEY
  ...
  public
    ...
    procedure SaveNZIS_QUESTIONNAIRE_RESPONSE(var dataPosition: Cardinal);
  ...
  public
	...
	ListForFDB: TList<TNZIS_QUESTIONNAIRE_RESPONSEItem>;
...
begin
  ...
  begin
	...
	ATempItem := TNZIS_QUESTIONNAIRE_RESPONSEColl(coll).ListForFDB.Items[0];
...
begin
  ...
  ListForFDB := TList<TNZIS_QUESTIONNAIRE_RESPONSEItem>.Create;
  FindedRes.DataPos := 0;
...
begin
  ...
  begin
    ...
    case propIndex of
      ...
      NZIS_QUESTIONNAIRE_RESPONSE_PR001_KEY: TempItem.IndexAnsiStr :=  TempItem.getPAnsiStringMap(Self.Buf, self.posData, word(propIndex), len);
...
function TNZIS_QUESTIONNAIRE_RESPONSEColl.PropType(propIndex: Word): TAsectTypeKind;
begin
  inherited;
  case TNZIS_QUESTIONNAIRE_RESPONSEItem.TPropertyIndex(propIndex) of
...
begin
  ...
  begin
    ListNZIS_QUESTIONNAIRE_RESPONSESearch.Add(self.Items[i]);
      ...
      NZIS_QUESTIONNAIRE_RESPONSE_PR001_KEY:
      begin
        if (self.Items[i].IndexAnsiStr) = FSearchingValue then
        begin
          ListNZIS_QUESTIONNAIRE_RESPONSESearch.Add(self.Items[i]);
        end;
      end;

...
, Pregled_VSD_TYPE
  ...
  public
    ...
    ListPregledSearch: TList<TPregledItem>;
    ...
    procedure SortByIndexValue(propIndex: TPregledItem.TPropertyIndex);
    ...
    procedure IndexValue(propIndex: TPregledItem.TPropertyIndex);
    property Items[Index: Integer]: TPregledItem read GetItem write SetItem;
    property SearchingValue: string read FSearchingValue write SetSearchingValue;
    //property SearchingInt: Integer read FSearchingInt write SetSearchingInt;
  end;
...
var
  ...
  pWordData: ^Word;
begin
  CollType := Aspects.Types.TCollectionsType.ctPregled;
...
constructor TPregledColl.Create(ItemClass: TCollectionItemClass);
begin
  inherited;
  ListPregledSearch := TList<TPregledItem>.Create;
  FindedRes.DataPos := 0;
...
procedure TPregledColl.DynControlEnter(Sender: TObject);
begin
  self.FindedRes.DataPos := 0;
  self.FindedRes.PropIndex := TBaseControl(sender).ColIndex;
  self.IndexValue(TPregledItem.TPropertyIndex(self.FindedRes.PropIndex));
  //Self.SortByIndexValue(TPregledItem.TPropertyIndex(self.FindedRes.PropIndex));;
end;

function TPregledColl.FieldCount: Integer;
begin
  Result := 105;
end;
...
procedure TPregledColl.GetCellSearch(Sender: TObject; const AColumn: TColumn; const ARow: Integer; var AValue: String);
var
  Pregled: TPregledItem;
  ACol: Integer;
  prop: TPregledItem.TPropertyIndex;
begin
  ...
  Pregled := ListPregledSearch[ARow];
  prop := TPregledItem.TPropertyIndex(ACol);
  if Assigned(Pregled.PRecord) and (prop in Pregled.PRecord.setProp) then
  begin
    GetCellFromRecord(ACol, Pregled, AValue);
...
procedure TPregledColl.GetFieldText(Sender: TObject; const ACol, ARow: Integer; var AFieldText: String);
var
  Pregled: TPregledItem;
  prop: TPregledItem.TPropertyIndex;
begin
  ...
  if Assigned(Pregled.PRecord) and (prop in Pregled.PRecord.setProp) then
  begin
    GetCellFromRecord(ACol, Pregled, AFieldText);
  end
  else
  begin
    GetCellFromMap(ACol, ARow, Pregled, AFieldText);
  end;
end;
...
begin
  case TPregledItem.TPropertyIndex(propIndex) of
    ...
    Pregled_DOM_PR: str :=  inttostr(Pregled.getWordMap(Self.Buf, Self.posData, propIndex));
    ...
    Pregled_GS: str :=  inttostr(Pregled.getWordMap(Self.Buf, Self.posData, propIndex));
    ...
    Pregled_NAPR_TYPE_ID: str :=  inttostr(Pregled.getWordMap(Self.Buf, Self.posData, propIndex));
    ...
    Pregled_NRD: str :=  inttostr(Pregled.getWordMap(Self.Buf, Self.posData, propIndex));
    Pregled_NRN: str :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, propIndex);
    Pregled_NZIS_STATUS: str :=  inttostr(Pregled.getWordMap(Self.Buf, Self.posData, propIndex));
    Pregled_OBSHTAPR: str :=  inttostr(Pregled.getWordMap(Self.Buf, Self.posData, propIndex));
    ...
    Pregled_PREVENTIVE_TYPE: str :=  inttostr(Pregled.getWordMap(Self.Buf, Self.posData, propIndex));
    ...
    Pregled_VISIT_TYPE_ID: str :=  inttostr(Pregled.getWordMap(Self.Buf, Self.posData, propIndex));
    Pregled_VSD_TYPE: str :=  inttostr(Pregled.getWordMap(Self.Buf, Self.posData, propIndex));
  else
    begin
      str := IntToStr(ARow + 1);
    end;
...
procedure TPregledColl.IndexValue(propIndex: TPregledItem.TPropertyIndex);
var
  i: Integer;
  len: Word;
  TempItem: TPregledItem;
begin
  for i := 0 to self.Count - 1 do
  begin
    TempItem := self.Items[i];
    case propIndex of
      Pregled_AMB_LISTN: TempItem.IndexInt := TempItem.getPIntMap(Self.Buf, self.posData, word(propIndex))^;
      Pregled_NRN,Pregled_ANAMN, Pregled_MAIN_DIAG_MKB:
      begin
...
begin
  ...
  begin
    ...
    case TPregledItem.TPropertyIndex(ACol) of
      Pregled_AMB_LISTN: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_AMB_PR: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_ANAMN: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_COPIED_FROM_NRN: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_DOCTOR_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_DOM_PR: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_EXAM_BOLN_LIST_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_FUND_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_GS: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_INCIDENTALLY: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_ANALYSIS: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_BABY_CARE: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_CONSULTATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_DISPANSERY: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_EMERGENCY: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_EPIKRIZA: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_EXPERTIZA: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_FORM_VALID: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_HOSPITALIZATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_MANIPULATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_MEDBELEJKA: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_NAET: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_NAPR_TELK: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_NEW: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_NOTIFICATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_NO_DELAY: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_OPERATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_PODVIZHNO_LZ: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_PREVENTIVE: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_PRINTED: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_IS_RECEPTA_HOSPIT: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_REGISTRATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_REHABILITATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_RISK_GROUP: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_TELK: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_VSD: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IS_ZAMESTVASHT: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_IZSL: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_LIFESENSOR_ID: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_LKK_KOMISII_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_MAIN_DIAG_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_MAIN_DIAG_MKB_ADD: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      //Pregled_MAIN_DIAG_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_MEDTRANSKM: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_NAPRAVLENIE_AMBL_NOMER: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_NAPR_TYPE_ID: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_NOMERBELEGKA: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_NOMERKASHAPARAT: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_NRD: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_NRN: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_NZIS_STATUS: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_OBSHTAPR: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_OWNER_DOCTOR_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_PACIENT_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_PATIENTOF_NEOTL: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_PATIENTOF_NEOTLID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_PAY: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_PREVENTIVE_TYPE: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_PRIMARY_NOTE_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_PROCEDURE1_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_PROCEDURE1_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      //Pregled_PROCEDURE1_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_PROCEDURE2_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_PROCEDURE2_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      //Pregled_PROCEDURE2_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_PROCEDURE3_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_PROCEDURE3_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      //Pregled_PROCEDURE3_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_PROCEDURE4_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_PROCEDURE4_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      //Pregled_PROCEDURE4_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_PR_ZAB1_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_PR_ZAB1_MKB_ADD: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      //Pregled_PR_ZAB1_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_PR_ZAB2_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_PR_ZAB2_MKB_ADD: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      //Pregled_PR_ZAB2_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_PR_ZAB3_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_PR_ZAB3_MKB_ADD: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      //Pregled_PR_ZAB3_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_PR_ZAB4_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_PR_ZAB4_MKB_ADD: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      //Pregled_PR_ZAB4_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_RECKNNO: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_REH_FINISHED_AT: isOld :=  Pregled.getDateMap(Self.Buf, Self.posData, ACol) = StrToDate(AValue);
      Pregled_REH_PROC1: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_REH_PROC2: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_REH_PROC3: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_REH_PROC4: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_SIMP_FORM_DATE: isOld :=  Pregled.getDateMap(Self.Buf, Self.posData, ACol) = StrToDate(AValue);
      Pregled_SIMP_IS_NAET: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_SIMP_IS_ZAMESTVASHT: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_SIMP_NAPR_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_SIMP_NAPR_N: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_SIMP_NAPR_NRN: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_SIMP_NZOKNOMER: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_SIMP_PRAKTIKA: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_SIMP_PRIMARY_AMBLIST_DATE: isOld :=  Pregled.getDateMap(Self.Buf, Self.posData, ACol) = StrToDate(AValue);
      Pregled_SIMP_PRIMARY_AMBLIST_N: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_SIMP_PRIMARY_AMBLIST_NRN: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_SIMP_SPECIALITY_CODE: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_SIMP_UIN: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_SIMP_UIN_ZAMESTVASHT: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_START_DATE: isOld :=  Pregled.getDateMap(Self.Buf, Self.posData, ACol) = StrToDate(AValue);
      Pregled_START_TIME: isOld :=  Pregled.getTimeMap(Self.Buf, Self.posData, ACol) = StrToTime(AValue);
      Pregled_SYST: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_TALON_LKK: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_TERAPY: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_THREAD_IDS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AValue;
      Pregled_TO_BE_DISPANSERED: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AValue);
      Pregled_VISIT_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_VISIT_TYPE_ID: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
      Pregled_VSD_TYPE: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AValue);
    end;
  end;
  if isOld then
  begin
    Exclude(Pregled.PRecord.setProp, TPregledItem.TPropertyIndex(ACol));
    if Pregled.PRecord.setProp = [] then
    begin
      Dispose(Pregled.PRecord);
      Pregled.PRecord := nil;
      CntUpdates := CntUpdates - 1;
      Exit;
    end;
  ...
  case TPregledItem.TPropertyIndex(ACol) of
    ...
    Pregled_SIMP_FORM_DATE: Pregled.PRecord.SIMP_FORM_DATE := StrToDate(AValue);
...
procedure TPregledColl.SetFieldText(Sender: TObject; const ACol, ARow: Integer; var AFieldText: String);
var
  isOld: Boolean;
  Pregled: TPregledItem;
begin
  ...
  begin
    isOld := False;
    case TPregledItem.TPropertyIndex(ACol) of
      Pregled_AMB_LISTN: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_AMB_PR: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_ANAMN: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_COPIED_FROM_NRN: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_DOCTOR_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_DOM_PR: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_EXAM_BOLN_LIST_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_FUND_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_GS: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_INCIDENTALLY: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_ANALYSIS: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_BABY_CARE: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_CONSULTATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_DISPANSERY: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_EMERGENCY: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_EPIKRIZA: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_EXPERTIZA: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_FORM_VALID: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_HOSPITALIZATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_MANIPULATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_MEDBELEJKA: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_NAET: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_NAPR_TELK: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_NEW: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_NOTIFICATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_NO_DELAY: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_OPERATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_PODVIZHNO_LZ: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_PREVENTIVE: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_PRINTED: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_IS_RECEPTA_HOSPIT: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_REGISTRATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_REHABILITATION: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_RISK_GROUP: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_TELK: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_VSD: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IS_ZAMESTVASHT: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_IZSL: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_LIFESENSOR_ID: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_LKK_KOMISII_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_MAIN_DIAG_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_MAIN_DIAG_MKB_ADD: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      //Pregled_MAIN_DIAG_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_MEDTRANSKM: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_NAPRAVLENIE_AMBL_NOMER: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_NAPR_TYPE_ID: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_NOMERBELEGKA: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_NOMERKASHAPARAT: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_NRD: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_NRN: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_NZIS_STATUS: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_OBSHTAPR: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_OWNER_DOCTOR_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_PACIENT_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_PATIENTOF_NEOTL: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_PATIENTOF_NEOTLID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_PAY: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_PREVENTIVE_TYPE: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_PRIMARY_NOTE_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_PROCEDURE1_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_PROCEDURE1_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      //Pregled_PROCEDURE1_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_PROCEDURE2_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_PROCEDURE2_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      //Pregled_PROCEDURE2_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_PROCEDURE3_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_PROCEDURE3_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      //Pregled_PROCEDURE3_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_PROCEDURE4_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_PROCEDURE4_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      //Pregled_PROCEDURE4_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_PR_ZAB1_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_PR_ZAB1_MKB_ADD: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      //Pregled_PR_ZAB1_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_PR_ZAB2_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_PR_ZAB2_MKB_ADD: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      //Pregled_PR_ZAB2_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_PR_ZAB3_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_PR_ZAB3_MKB_ADD: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      //Pregled_PR_ZAB3_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_PR_ZAB4_MKB: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_PR_ZAB4_MKB_ADD: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      //Pregled_PR_ZAB4_OPIS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_RECKNNO: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_REH_FINISHED_AT: isOld :=  Pregled.getDateMap(Self.Buf, Self.posData, ACol) = StrToDate(AFieldText);
      Pregled_REH_PROC1: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_REH_PROC2: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_REH_PROC3: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_REH_PROC4: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_SIMP_FORM_DATE: isOld :=  Pregled.getDateMap(Self.Buf, Self.posData, ACol) = StrToDate(AFieldText);
      Pregled_SIMP_IS_NAET: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_SIMP_IS_ZAMESTVASHT: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_SIMP_NAPR_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_SIMP_NAPR_N: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_SIMP_NAPR_NRN: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_SIMP_NZOKNOMER: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_SIMP_PRAKTIKA: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_SIMP_PRIMARY_AMBLIST_DATE: isOld :=  Pregled.getDateMap(Self.Buf, Self.posData, ACol) = StrToDate(AFieldText);
      Pregled_SIMP_PRIMARY_AMBLIST_N: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_SIMP_PRIMARY_AMBLIST_NRN: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_SIMP_SPECIALITY_CODE: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_SIMP_UIN: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_SIMP_UIN_ZAMESTVASHT: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_START_DATE: isOld :=  Pregled.getDateMap(Self.Buf, Self.posData, ACol) = StrToDate(AFieldText);
      Pregled_START_TIME: isOld :=  Pregled.getTimeMap(Self.Buf, Self.posData, ACol) = StrToTime(AFieldText);
      Pregled_SYST: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_TALON_LKK: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_TERAPY: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_THREAD_IDS: isOld :=  Pregled.getAnsiStringMap(Self.Buf, Self.posData, ACol) = AFieldText;
      Pregled_TO_BE_DISPANSERED: isOld :=  Pregled.getBooleanMap(Self.Buf, Self.posData, ACol) = StrToBool(AFieldText);
      Pregled_VISIT_ID: isOld :=  Pregled.getIntMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_VISIT_TYPE_ID: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
      Pregled_VSD_TYPE: isOld :=  Pregled.getWordMap(Self.Buf, Self.posData, ACol) = StrToInt(AFieldText);
    end;
  end;
  if isOld then
  begin
  ...
  Include(Pregled.PRecord.setProp, TPregledItem.TPropertyIndex(ACol));
  case TPregledItem.TPropertyIndex(ACol) of
    Pregled_AMB_LISTN: Pregled.PRecord.AMB_LISTN := StrToInt(AFieldText);
    Pregled_AMB_PR: Pregled.PRecord.AMB_PR := StrToInt(AFieldText);
    Pregled_ANAMN: Pregled.PRecord.ANAMN := AFieldText;
    Pregled_COPIED_FROM_NRN: Pregled.PRecord.COPIED_FROM_NRN := AFieldText;
    Pregled_DOCTOR_ID: Pregled.PRecord.DOCTOR_ID := StrToInt(AFieldText);
    Pregled_DOM_PR: Pregled.PRecord.DOM_PR := StrToInt(AFieldText);
    Pregled_EXAM_BOLN_LIST_ID: Pregled.PRecord.EXAM_BOLN_LIST_ID := StrToInt(AFieldText);
    Pregled_FUND_ID: Pregled.PRecord.FUND_ID := StrToInt(AFieldText);
    Pregled_GS: Pregled.PRecord.GS := StrToInt(AFieldText);
    Pregled_ID: Pregled.PRecord.ID := StrToInt(AFieldText);
    Pregled_INCIDENTALLY: Pregled.PRecord.INCIDENTALLY := StrToBool(AFieldText);
    Pregled_IS_ANALYSIS: Pregled.PRecord.IS_ANALYSIS := StrToBool(AFieldText);
    Pregled_IS_BABY_CARE: Pregled.PRecord.IS_BABY_CARE := StrToBool(AFieldText);
    Pregled_IS_CONSULTATION: Pregled.PRecord.IS_CONSULTATION := StrToBool(AFieldText);
    Pregled_IS_DISPANSERY: Pregled.PRecord.IS_DISPANSERY := StrToBool(AFieldText);
    Pregled_IS_EMERGENCY: Pregled.PRecord.IS_EMERGENCY := StrToBool(AFieldText);
    Pregled_IS_EPIKRIZA: Pregled.PRecord.IS_EPIKRIZA := StrToBool(AFieldText);
    Pregled_IS_EXPERTIZA: Pregled.PRecord.IS_EXPERTIZA := StrToBool(AFieldText);
    Pregled_IS_FORM_VALID: Pregled.PRecord.IS_FORM_VALID := StrToBool(AFieldText);
    Pregled_IS_HOSPITALIZATION: Pregled.PRecord.IS_HOSPITALIZATION := StrToBool(AFieldText);
    Pregled_IS_MANIPULATION: Pregled.PRecord.IS_MANIPULATION := StrToBool(AFieldText);
    Pregled_IS_MEDBELEJKA: Pregled.PRecord.IS_MEDBELEJKA := StrToBool(AFieldText);
    Pregled_IS_NAET: Pregled.PRecord.IS_NAET := StrToBool(AFieldText);
    Pregled_IS_NAPR_TELK: Pregled.PRecord.IS_NAPR_TELK := StrToBool(AFieldText);
    Pregled_IS_NEW: Pregled.PRecord.IS_NEW := StrToBool(AFieldText);
    Pregled_IS_NOTIFICATION: Pregled.PRecord.IS_NOTIFICATION := StrToBool(AFieldText);
    Pregled_IS_NO_DELAY: Pregled.PRecord.IS_NO_DELAY := StrToBool(AFieldText);
    Pregled_IS_OPERATION: Pregled.PRecord.IS_OPERATION := StrToBool(AFieldText);
    Pregled_IS_PODVIZHNO_LZ: Pregled.PRecord.IS_PODVIZHNO_LZ := StrToBool(AFieldText);
    Pregled_IS_PREVENTIVE: Pregled.PRecord.IS_PREVENTIVE := StrToBool(AFieldText);
    Pregled_IS_PRINTED: Pregled.PRecord.IS_PRINTED := AFieldText;
    Pregled_IS_RECEPTA_HOSPIT: Pregled.PRecord.IS_RECEPTA_HOSPIT := StrToBool(AFieldText);
    Pregled_IS_REGISTRATION: Pregled.PRecord.IS_REGISTRATION := StrToBool(AFieldText);
    Pregled_IS_REHABILITATION: Pregled.PRecord.IS_REHABILITATION := StrToBool(AFieldText);
    Pregled_IS_RISK_GROUP: Pregled.PRecord.IS_RISK_GROUP := StrToBool(AFieldText);
    Pregled_IS_TELK: Pregled.PRecord.IS_TELK := StrToBool(AFieldText);
    Pregled_IS_VSD: Pregled.PRecord.IS_VSD := StrToBool(AFieldText);
    Pregled_IS_ZAMESTVASHT: Pregled.PRecord.IS_ZAMESTVASHT := StrToBool(AFieldText);
    Pregled_IZSL: Pregled.PRecord.IZSL := AFieldText;
    Pregled_LIFESENSOR_ID: Pregled.PRecord.LIFESENSOR_ID := AFieldText;
    Pregled_LKK_KOMISII_ID: Pregled.PRecord.LKK_KOMISII_ID := StrToInt(AFieldText);
    Pregled_MAIN_DIAG_MKB: Pregled.PRecord.MAIN_DIAG_MKB := AFieldText;
    Pregled_MAIN_DIAG_MKB_ADD: Pregled.PRecord.MAIN_DIAG_MKB_ADD := AFieldText;
    //Pregled_MAIN_DIAG_OPIS: Pregled.PRecord.MAIN_DIAG_OPIS := AFieldText;
    Pregled_MEDTRANSKM: Pregled.PRecord.MEDTRANSKM := StrToInt(AFieldText);
    Pregled_NAPRAVLENIE_AMBL_NOMER: Pregled.PRecord.NAPRAVLENIE_AMBL_NOMER := AFieldText;
    Pregled_NAPR_TYPE_ID: Pregled.PRecord.NAPR_TYPE_ID := StrToInt(AFieldText);
    Pregled_NOMERBELEGKA: Pregled.PRecord.NOMERBELEGKA := AFieldText;
    Pregled_NOMERKASHAPARAT: Pregled.PRecord.NOMERKASHAPARAT := AFieldText;
    Pregled_NRD: Pregled.PRecord.NRD := StrToInt(AFieldText);
    Pregled_NRN: Pregled.PRecord.NRN := AFieldText;
    Pregled_NZIS_STATUS: Pregled.PRecord.NZIS_STATUS := StrToInt(AFieldText);
    Pregled_OBSHTAPR: Pregled.PRecord.OBSHTAPR := StrToInt(AFieldText);
    Pregled_OWNER_DOCTOR_ID: Pregled.PRecord.OWNER_DOCTOR_ID := StrToInt(AFieldText);
    Pregled_PACIENT_ID: Pregled.PRecord.PACIENT_ID := StrToInt(AFieldText);
    Pregled_PATIENTOF_NEOTL: Pregled.PRecord.PATIENTOF_NEOTL := AFieldText;
    Pregled_PATIENTOF_NEOTLID: Pregled.PRecord.PATIENTOF_NEOTLID := StrToInt(AFieldText);
    Pregled_PAY: Pregled.PRecord.PAY := StrToBool(AFieldText);
    Pregled_PREVENTIVE_TYPE: Pregled.PRecord.PREVENTIVE_TYPE := StrToInt(AFieldText);
    Pregled_PRIMARY_NOTE_ID: Pregled.PRecord.PRIMARY_NOTE_ID := StrToInt(AFieldText);
    Pregled_PROCEDURE1_ID: Pregled.PRecord.PROCEDURE1_ID := StrToInt(AFieldText);
    Pregled_PROCEDURE1_MKB: Pregled.PRecord.PROCEDURE1_MKB := AFieldText;
    //Pregled_PROCEDURE1_OPIS: Pregled.PRecord.PROCEDURE1_OPIS := AFieldText;
    Pregled_PROCEDURE2_ID: Pregled.PRecord.PROCEDURE2_ID := StrToInt(AFieldText);
    Pregled_PROCEDURE2_MKB: Pregled.PRecord.PROCEDURE2_MKB := AFieldText;
    //Pregled_PROCEDURE2_OPIS: Pregled.PRecord.PROCEDURE2_OPIS := AFieldText;
    Pregled_PROCEDURE3_ID: Pregled.PRecord.PROCEDURE3_ID := StrToInt(AFieldText);
    Pregled_PROCEDURE3_MKB: Pregled.PRecord.PROCEDURE3_MKB := AFieldText;
    //Pregled_PROCEDURE3_OPIS: Pregled.PRecord.PROCEDURE3_OPIS := AFieldText;
    Pregled_PROCEDURE4_ID: Pregled.PRecord.PROCEDURE4_ID := StrToInt(AFieldText);
    Pregled_PROCEDURE4_MKB: Pregled.PRecord.PROCEDURE4_MKB := AFieldText;
    //Pregled_PROCEDURE4_OPIS: Pregled.PRecord.PROCEDURE4_OPIS := AFieldText;
    Pregled_PR_ZAB1_MKB: Pregled.PRecord.PR_ZAB1_MKB := AFieldText;
    Pregled_PR_ZAB1_MKB_ADD: Pregled.PRecord.PR_ZAB1_MKB_ADD := AFieldText;
    //Pregled_PR_ZAB1_OPIS: Pregled.PRecord.PR_ZAB1_OPIS := AFieldText;
    Pregled_PR_ZAB2_MKB: Pregled.PRecord.PR_ZAB2_MKB := AFieldText;
    Pregled_PR_ZAB2_MKB_ADD: Pregled.PRecord.PR_ZAB2_MKB_ADD := AFieldText;
    //Pregled_PR_ZAB2_OPIS: Pregled.PRecord.PR_ZAB2_OPIS := AFieldText;
    Pregled_PR_ZAB3_MKB: Pregled.PRecord.PR_ZAB3_MKB := AFieldText;
    Pregled_PR_ZAB3_MKB_ADD: Pregled.PRecord.PR_ZAB3_MKB_ADD := AFieldText;
    //Pregled_PR_ZAB3_OPIS: Pregled.PRecord.PR_ZAB3_OPIS := AFieldText;
    Pregled_PR_ZAB4_MKB: Pregled.PRecord.PR_ZAB4_MKB := AFieldText;
    Pregled_PR_ZAB4_MKB_ADD: Pregled.PRecord.PR_ZAB4_MKB_ADD := AFieldText;
    //Pregled_PR_ZAB4_OPIS: Pregled.PRecord.PR_ZAB4_OPIS := AFieldText;
    Pregled_RECKNNO: Pregled.PRecord.RECKNNO := AFieldText;
    Pregled_REH_FINISHED_AT: Pregled.PRecord.REH_FINISHED_AT := StrToDate(AFieldText);
    Pregled_REH_PROC1: Pregled.PRecord.REH_PROC1 := StrToInt(AFieldText);
    Pregled_REH_PROC2: Pregled.PRecord.REH_PROC2 := StrToInt(AFieldText);
    Pregled_REH_PROC3: Pregled.PRecord.REH_PROC3 := StrToInt(AFieldText);
    Pregled_REH_PROC4: Pregled.PRecord.REH_PROC4 := StrToInt(AFieldText);
    Pregled_SIMP_FORM_DATE: Pregled.PRecord.SIMP_FORM_DATE := StrToDate(AFieldText);
    Pregled_SIMP_IS_NAET: Pregled.PRecord.SIMP_IS_NAET := StrToBool(AFieldText);
    Pregled_SIMP_IS_ZAMESTVASHT: Pregled.PRecord.SIMP_IS_ZAMESTVASHT := StrToBool(AFieldText);
    Pregled_SIMP_NAPR_ID: Pregled.PRecord.SIMP_NAPR_ID := StrToInt(AFieldText);
    Pregled_SIMP_NAPR_N: Pregled.PRecord.SIMP_NAPR_N := StrToInt(AFieldText);
    Pregled_SIMP_NAPR_NRN: Pregled.PRecord.SIMP_NAPR_NRN := AFieldText;
    Pregled_SIMP_NZOKNOMER: Pregled.PRecord.SIMP_NZOKNOMER := AFieldText;
    Pregled_SIMP_PRAKTIKA: Pregled.PRecord.SIMP_PRAKTIKA := AFieldText;
    Pregled_SIMP_PRIMARY_AMBLIST_DATE: Pregled.PRecord.SIMP_PRIMARY_AMBLIST_DATE := StrToDate(AFieldText);
    Pregled_SIMP_PRIMARY_AMBLIST_N: Pregled.PRecord.SIMP_PRIMARY_AMBLIST_N := StrToInt(AFieldText);
    Pregled_SIMP_PRIMARY_AMBLIST_NRN: Pregled.PRecord.SIMP_PRIMARY_AMBLIST_NRN := AFieldText;
    Pregled_SIMP_SPECIALITY_CODE: Pregled.PRecord.SIMP_SPECIALITY_CODE := AFieldText;
    Pregled_SIMP_UIN: Pregled.PRecord.SIMP_UIN := AFieldText;
    Pregled_SIMP_UIN_ZAMESTVASHT: Pregled.PRecord.SIMP_UIN_ZAMESTVASHT := AFieldText;
    Pregled_START_DATE: Pregled.PRecord.START_DATE := StrToDate(AFieldText);
    Pregled_START_TIME: Pregled.PRecord.START_TIME := StrToTime(AFieldText);
    Pregled_SYST: Pregled.PRecord.SYST := AFieldText;
    Pregled_TALON_LKK: Pregled.PRecord.TALON_LKK := AFieldText;
    Pregled_TERAPY: Pregled.PRecord.TERAPY := AFieldText;
    Pregled_THREAD_IDS: Pregled.PRecord.THREAD_IDS := AFieldText;
    Pregled_TO_BE_DISPANSERED: Pregled.PRecord.TO_BE_DISPANSERED := StrToBool(AFieldText);
    Pregled_VISIT_ID: Pregled.PRecord.VISIT_ID := StrToInt(AFieldText);
    Pregled_VISIT_TYPE_ID: Pregled.PRecord.VISIT_TYPE_ID := StrToInt(AFieldText);
    Pregled_VSD_TYPE: Pregled.PRecord.VSD_TYPE := StrToInt(AFieldText);
  end;
end;
...
procedure TPregledColl.SetSearchingValue(const Value: string);
var
  i: Integer;
begin
  FSearchingValue := Value;
  ListPregledSearch.Clear;
  for i := 0 to self.Count - 1 do
  begin
    case  TPregledItem.TPropertyIndex(self.FindedRes.PropIndex) of
	  Pregled_AMB_LISTN:
      begin
...
procedure TPregledColl.ShowSearchedGrid(Grid: TTeeGrid);
var
  i: word;

begin
  Grid.Data:=TVirtualModeData.Create(self.FieldCount + 1, self.ListPregledSearch.Count);
  for i := 0 to self.FieldCount - 1 do
  begin
    TVirtualModeData(Grid.Data).Headers[i] := self.DisplayName(i);
  end;
  TVirtualModeData(Grid.Data).Headers[self.FieldCount] := Format('Ред/%d бр.', [self.ListPregledSearch.Count]);
...
procedure TPregledColl.SortByIndexAnsiString;
var
...
procedure TPregledColl.SortByIndexInt;
var
  sc : TList<TCollectionItem>;
...
procedure TPregledColl.SortByIndexValue(propIndex: TPregledItem.TPropertyIndex);
begin
  case propIndex of
    Pregled_AMB_LISTN:
    begin

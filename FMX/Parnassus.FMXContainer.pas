...
type
  ...
  public
    ...
    procedure ChangeActiveForm(FormForShow: FMX.Forms.TCommonCustomForm);
...
procedure TFireMonkeyContainer.ChangeActiveForm(FormForShow: FMX.Forms.TCommonCustomForm);
begin
  if not (self.FireMonkeyForm = FormForShow) then
  begin
    if self.FireMonkeyForm <> nil then
    begin
      self.FireMonkeyForm.Focused := nil;
      self.FireMonkeyForm.Hide;
    end;
    FormForShow.Focused := nil;
    self.FireMonkeyForm := FormForShow;
    self.FireMonkeyForm.Show;
  end;
end;
...
begin
  ...
  case Msg.Msg of
    WM_ACTIVATE, WM_MOUSEACTIVATE: begin // Activate when clicked on
      ...
      begin
        if Assigned(FOnActivateForm) then
          FOnActivateForm(FFMXForm);
      end;
      CallOriginal := false;
    end;
